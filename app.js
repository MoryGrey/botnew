...
function validateBeforeOrder() {
  ...
  if (!state.profile.phone) {
    alert('Укажите актуальный номер телефона в профиле');
...
function initTelegram() {
  ...
  try {
    ...
    state.profile.username = u.username ? `@${u.username}` : state.profile.username;
    saveProfile();
    updateProfileUI();
...
function bindUI() {
  ...
  qs('#edit-profile')?.addEventListener('click', () => {
    const name = prompt('Введите имя', state.profile.name || '');
    if (name !== null) state.profile.name = name.trim();
    const phone = prompt('Введите телефон', state.profile.phone || '');
    if (phone !== null) state.profile.phone = phone.trim();
    updateProfileUI();
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.Telegram?.WebApp) initTelegram();
    ...
    const iv = setInterval(() => {
      if (window.Telegram?.WebApp) { clearInterval(iv); initTelegram(); }
    }, 300);
