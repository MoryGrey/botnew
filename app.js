...
function bindUI() {
  ...
  testBtn.textContent = 'Тест данных';
  testBtn.style.cssText = 'position:fixed;top:10px;right:10px;z-index:9999;background:#007bff;color:white;border:none;padding:5px 10px;border-radius:5px;';
  testBtn.addEventListener('click', simulateBotData);
...
// Функция для имитации получения данных от бота (для тестирования)
function simulateBotData() {
  // Имитируем данные от бота
  const mockData = {
    type: 'user_data',
  ...
  handleBotMessage(mockData);
  console.log('Имитация данных от бота:', mockData);
}
